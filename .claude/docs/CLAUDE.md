...
### 1. 인증 및 팀 관리
- **계정 시스템**: ID/비밀번호 기반 로그인 (보안을 위해 자동 로그인 제거, ID만 기억)
- **팀 시스템**:
  - 6자리 팀코드를 통한 팀 생성/참여 (예: ABC123)
  - 팀 생성자(creator)와 일반 멤버(member) 역할 구분
  - ID로 직접 팀원 초대 기능 (이미 다른 팀에 소속된 사용자는 초대 불가)
  - 팀원 초대 알림 시스템 (초대 수락/거절 가능)
...
- **그라디언트 디자인**:
  ...
  - 로그인 화면: `#667eea` → `#764ba2` (보라색 계열)
  - 모던하고 전문적인 느낌
- **반응형 디자인**: PC/모바일 모두 지원
- **모바일 최적화**:
  - 키보드 올라와도 버튼 표시되도록 모달 상단 정렬
  - 터치 영역 충분히 확보
  - 스와이프 제스처 지원
...
- **모달 시스템**:
  ...
  - 모바일에서 overflow-y: auto로 스크롤 가능
- **간결한 사용설명서**: 핵심만 남긴 14단계 가이드
...
3. **보안**
   - ✅ 자동 로그인 제거로 보안 강화 (ID만 기억, 비밀번호는 매번 입력)
   - ⚠️ 비밀번호 평문 저장 (해시 처리 필요)
   - ⚠️ Firebase Security Rules 강화 필요
   - ⚠️ Firebase API 키가 클라이언트에 노출됨

#### 🟡 중요 개선 사항
4. **UX 개선**
   - ✅ 모바일 키보드 처리 개선 (모달 상단 정렬, 버튼 가려짐 해결)
   - ✅ 자동 경로 표시/숨김 (작업 유무에 따라)
   - ✅ 로그인 화면 배경 개선 (보라색 그라디언트)
   - ⚠️ 작업 순서 변경 후 경로가 자동으로 재계산되지 않음
   - ⚠️ 오프라인 모드 미지원 (PWA 전환 고려)
...
- 💬 카카오톡: @durian0606
- 빠른 응답을 원하시면 카카오톡으로 연락주세요!

---

**마지막 업데이트**: 2025-11-15
**프로젝트 상태**: 활성 개발 중
**현재 버전**: v0.9.7 (배포 환경 운영 중)
