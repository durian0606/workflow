...
import {
    ...
    window.loginCompany = async function() {
      ...
      try {
        ...
        console.log('✅ 관리자 찾음:', adminUser.name);
    ...
    window.createCompany = async function() {
      ...
      try {
        ...
        alert(
          ...
          `회사 ID: ${companyId}\n` +
          `관리자: ${adminName}\n` +
          `회사 코드: ${newCompanyCode}`
        );
        
        console.log('🎉 회사 생성 완료! 메인 앱으로 이동...');
      ...
      if (work.work === '시험') {
        ...
        Object.keys(works).forEach(id => {
          if (works[id].parentWorkId === workId) {
            const childWorkRef = window.dbRef(window.db, `companies/${currentCompanyId}/works/${id}`);
